...
def main(args):
    ...
    def file_name(prefix, lang):
        return prefix if lang is None else "{}.{}".format(prefix, lang)
    ...
    def build_dictionary(filenames, src=False, tgt=False, fileformat=None):
        assert src ^ tgt
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers):
        ...
        input_file = input_prefix if lang is None else "{}.{}".format(input_prefix, lang)
        ...
        merge_result(Binarizer.binarize(
        ...
        print("| [{}] {}: {} sents, {} tokens, {:.3}% replaced by {}".format(
              ...
              100 * sum(replaced.values()) / n_seq_tok[1], vocab.unk_word))
    ...
    def make_binary_alignment_dataset(input_prefix, output_prefix, num_workers):
        ...
        tk = tokenize_smiles if args.file_format == 'smiles' else tokenize_line
        merge_result(Binarizer.binarize_alignments(
    ...
    def make_dataset(vocab, input_prefix, output_prefix, lang, num_workers=1):
        if args.dataset_impl == "raw":
            ...
            output_text_file = dest_path(
                output_prefix + ".{}-{}".format(args.source_lang, args.target_lang), lang)
    ...
    def make_all(lang, vocab):
        if args.trainpref:
            make_dataset(vocab, args.trainpref, args.trainoutf, lang, num_workers=args.workers)
        if args.validpref:
            for k, validpref in enumerate(args.validpref.split(",")):
                outprefix = "{}{}".format(args.validoutf, k) if k > 0 else args.validoutf
                make_dataset(vocab, validpref, outprefix, lang, num_workers=args.workers)
        if args.testpref:
            for k, testpref in enumerate(args.testpref.split(",")):
                outprefix = "{}{}".format(args.testoutf, k) if k > 0 else args.testoutf
    ...
    if args.alignfile:
        ...
        with open(os.path.join(args.destdir,
...
def binarize(args, filename, vocab, output_prefix, lang, offset, end, append_eos=True):
    ...
    def consumer(tensor):
        ds.add_item(tensor)
    tk = tokenize_smiles if args.file_format == 'smiles' else tokenize_line
    res = Binarizer.binarize(filename, vocab, consumer, tokenize=tk,
...
def binarize_alignments(args, filename, parse_alignment, output_prefix, offset, end):
    ...
    def consumer(tensor):
        ds.add_item(tensor)
    tk = tokenize_smiles if args.file_format == 'smiles' else tokenize_line
    res = Binarizer.binarize_alignments(filename, parse_alignment, consumer, tokenize=tk,
